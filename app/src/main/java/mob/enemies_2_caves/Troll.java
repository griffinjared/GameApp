...
public class Troll extends Enemy {
    ...
    public Troll(int x, int y, Context context, Player player) {
        super(x, y, player);

        System.gc();
        Bitmap sprites = BitmapFactory.decodeResource(context.getResources(), R.drawable.forest_enemy_troll);
        setSprites(sprites);
    ...
    public void update(Room room) {
        ...
        if (Math.abs(currentTime - timer) > 1500) {
            timer = currentTime;
